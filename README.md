...
本答案为民间制作，并非官方标准答案.如有疏漏之处，欢迎各位大佬批评指正（通过 issue 或者 pull request），答案才刚刚开始写，预计 3 月底会把前半本书肝完.
...
【注】对于 C 项，同学们主要记住 `\ooo` 的表示方式上限为 `\377`（十进制的 `255` ），具体细节可以暂且不深究。
...
对于表达式 `A ? B : C` ，当`B`和`C`的类型不相同时，将按照C++相关标准进行类型转换.
